...
export const EquipmentStateHistory = ({
  ...
  const filterEquipmentState = filterEquipmentStateHistory.map(
    (equipmentState) => {
  ...
  if (activeHistory)
    return (
      <>
        <button
          disabled={equipmentID == false}
          className={styles.occultHistory}
          onClick={() => setActiveHistory(false)}
        >
          Ocultar histórico do equipamento
        </button>

        <div className={styles.container}>
          <h3>
            Histórico de estados do equipamento: {filterEquipment[0].name}
          </h3>
          {filterEquipmentState.map((states) => {
            return states.map((state) => {
              const filterStates = equipmentStateList.filter(
                (equipmentState) => {
  ...
  return (
    <button
      disabled={equipmentID == false}
      className={`${styles.historyButton} ${equipmentID && "btnHistoryActive"}`}
      onClick={() => setActiveHistory(true)}
    >
      Exibir histórico do equipamento
    </button>
  );
};
